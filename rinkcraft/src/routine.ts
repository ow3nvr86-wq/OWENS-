...
const EQUIPMENT_RANK: Record<Equipment, number> = { none: 0, stick: 1, pad: 2, ice: 3, gym: 4 };
...
export function estimateMinutes(drill: Drill): number {
  const baseline: Record<Focus, number> = {
    stickhandling: 5, shooting: 6, passing: 5, skating: 5,
    agility: 5, power: 6, goalie: 5, conditioning: 7,
  };
...
function chosenFocus(profile: Profile): Focus[] {
  ...
  return picked.length ? picked : (['stickhandling', 'shooting', 'conditioning'] as Focus[]);
}
...
export function buildRoutine(profile: Profile): RoutineDay[] {
  ...
  const byFocus = (f: Focus) =>
    usable
      .filter((d) => d.focus === f && d.id !== 'cooldown-hockey')
  ...
  const cooldown = DRILLS.find((d) => d.id === 'cooldown-hockey');
