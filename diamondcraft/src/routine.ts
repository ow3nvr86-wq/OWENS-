...
const EQUIPMENT_RANK: Record<Equipment, number> = { none: 0, ball: 1, tee: 2, field: 3, gym: 4 };
...
export function estimateMinutes(drill: Drill): number {
  const baseline: Record<Focus, number> = {
    hitting: 6, throwing: 5, fielding: 5, pitching: 6,
    catching: 5, speed: 5, power: 6, conditioning: 7,
  };
...
function chosenFocus(profile: Profile): Focus[] {
  ...
  return picked.length ? picked : (['hitting', 'throwing', 'conditioning'] as Focus[]);
}
...
export function buildRoutine(profile: Profile): RoutineDay[] {
  ...
  const byFocus = (f: Focus) =>
    usable
      .filter((d) => d.focus === f && d.id !== 'cooldown-bb')
  ...
  const cooldown = DRILLS.find((d) => d.id === 'cooldown-bb');
