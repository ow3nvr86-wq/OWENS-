...
export function estimateMinutes(drill: Drill): number {
  const baseline: Record<Focus, number> = {
    passing: 5, setting: 5, hitting: 6, serving: 5,
    blocking: 5, defense: 5, jumping: 6, conditioning: 7,
  };
...
function chosenFocus(profile: Profile): Focus[] {
  ...
  return picked.length ? picked : (['passing', 'hitting', 'conditioning'] as Focus[]);
}
...
export function buildRoutine(profile: Profile): RoutineDay[] {
  ...
  const byFocus = (f: Focus) =>
    usable
      .filter((d) => d.focus === f && d.id !== 'cooldown-vb')
  ...
  const cooldown = DRILLS.find((d) => d.id === 'cooldown-vb');
